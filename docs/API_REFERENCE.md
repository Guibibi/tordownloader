...
| `/v1/api/queued/controlqueued` | POST | `{queued_id, operation, type}` (`type="torrent"`). We use `delete` to clean up a download still in TorBox's **queue** — a queued download is a separate namespace and `controltorrent` will **not** touch it. **Verified live 2026-06-15.** |
